...
bool parse_pxb_header(const std::vector<uint8_t>& payload, PxbHeader& h) {
    ...
    if (meta) {
        ...
        // Newer exports reference the document image as an inline data URI
        // instead of a preview_table blob (see parse_pxb_header tail).
        std::string turl = meta->get("thumbnail_url");
        if (turl.compare(0, 5, "data:") == 0) m.thumbnail_data_uri = std::move(turl);
    }
    ...
    // preview_table is OPTIONAL: newer MZB.ONE exports (container still 2.x)
    // replace it with a `texture_table` + an inline `metadata.thumbnail_url`
    // data URI, and carry per-frame previews inside the source region. A file
    // without preview_table is therefore valid — the full reader falls back to
    // the data-URI image (see parse_pxb_payload), and the lazy paths below use
    // thumbnail_data_uri.
...
bool read_meta_and_thumb(const std::vector<uint8_t>& file_bytes,
    ...
    // else the inline metadata.thumbnail_url data URI (no-preview_table files).
    ...
    if (h.thumb_kind == 0) { blob_off = h.thumb_off; blob_len = h.thumb_len; }
        ...
        if (h.meta.thumbnail_data_uri.empty()) return true;   // metadata only
        RgbaImage img = decode_data_uri_rgba(h.meta.thumbnail_data_uri);
        if (img.empty()) return true;
        thumb_out = fit_square_nearest(img, thumb_n);
        return true;
    }
    ...
    // Thumbnails are always square canvases with the art letterboxed inside
    // (fit_square_nearest) — rectangular images must never be squeezed, and
    // every consumer (app list, Explorer, Linux thumbnailer) expects a square.
